 ...
 * Which fields are supported is defined by the PeriodType class. The default is
 * the standard period type, which supports years, months, weeks, days, hours,
 * minutes, seconds and millis.
 * <p>
 * When this time period is added to an instant, the effect is of adding each
 * field in turn. As a result, this takes into account daylight savings time.
 * Adding a time period of 1 day to the day before daylight savings starts will
 * only add 23 hours rather than 24 to ensure that the time remains the same. If
 * this is not the behaviour you want, then see {@link Duration}.
 * <p>
 * The definition of a period also affects the equals method. A period of 1 day
 * is not equal to a period of 24 hours, nor 1 hour equal to 60 minutes. This is
 * because periods represent an abstracted definition of a time period (eg. a
 * day may not actually be 24 hours, it might be 23 or 25 at daylight savings
 * boundary). To compare the actual duration of two periods, convert both to
 * durations using toDuration, an operation that emphasises that the result may
...
public final class Period extends BasePeriod implements ReadablePeriod, Serializable {
   ...
   * <p>
   * The standard period type is used, thus you can add other fields such as
   * months or days using the <code>withXxx()</code> methods. For example,
   * <code>Period.years(2).withMonths(6);</code>
   * <p>
   * If you want a year-based period that cannot have other fields added, then
   * you should consider using {@link Years}.
   ...
   * <p>
   * The standard period type is used, thus you can add other fields such as
   * years or days using the <code>withXxx()</code> methods. For example,
   * <code>Period.months(2).withDays(6);</code>
   * <p>
   * If you want a month-based period that cannot have other fields added, then
   * you should consider using {@link Months}.
   ...
   * The standard period type is used, thus you can add other fields such as
   * months or days using the <code>withXxx()</code> methods. For example,
   * <code>Period.weeks(2).withDays(6);</code>
   * <p>
   * If you want a week-based period that cannot have other fields added, then
   * you should consider using {@link Weeks}.
   ...
   * <p>
   * The standard period type is used, thus you can add other fields such as
   * months or weeks using the <code>withXxx()</code> methods. For example,
   * <code>Period.days(2).withHours(6);</code>
   * <p>
   * If you want a day-based period that cannot have other fields added, then
   * you should consider using {@link Days}.
   ...
   * The standard period type is used, thus you can add other fields such as
   * months or days using the <code>withXxx()</code> methods. For example,
   * <code>Period.hours(2).withMinutes(30);</code>
   * <p>
   * If you want a hour-based period that cannot have other fields added, then
   * you should consider using {@link Hours}.
   ...
   * <p>
   * The standard period type is used, thus you can add other fields such as
   * days or hours using the <code>withXxx()</code> methods. For example,
   * <code>Period.minutes(2).withSeconds(30);</code>
   * <p>
   * If you want a minute-based period that cannot have other fields added, then
   * you should consider using {@link Minutes}.
   ...
   * The standard period type is used, thus you can add other fields such as
   * days or hours using the <code>withXxx()</code> methods. For example,
   * <code>Period.seconds(2).withMillis(30);</code>
   * <p>
   * If you want a second-based period that cannot have other fields added, then
   * you should consider using {@link Seconds}.
   ...
   * The standard period type is used, thus you can add other fields such as
   * days or hours using the <code>withXxx()</code> methods. For example,
   * <code>Period.millis(20).withSeconds(30);</code>
  ...
  //-----------------------------------------------------------------------
  /**
   * Creates a period from two partially specified times, calculating by field
   * difference.
   * <p>
   * The two partials must contain the same fields, thus you can specify two
   * <code>LocalDate</code> objects, or two <code>LocalTime</code> objects, but
   * not one of each. Also, the partial may not contain overlapping fields, such
   * as dayOfWeek and dayOfMonth.
   * <p>
   * Calculation by field difference works by extracting the difference one
   * field at a time and not wrapping into other fields. Thus
   * 2005-06-09/2007-04-12 will yield P1Y-2M3D.
   * <p>
   * For example, you have an event that always runs from the 27th of each month
   * to the 2nd of the next month. If you calculate this period using a standard
   * constructor, then you will get between P3D and P6D depending on the month.
   * If you use this method, then you will get P1M-25D. This field-difference
   * based period can be successfully applied to each month of the year to
   ...
   * which are immediately discarded. The alterative is more efficient, but less
   * readable:
   * 
   * <pre>
  ...
  /**
   * Create a period from a set of field values using the standard set of
   * fields. Note that the parameters specify the time fields hours, minutes,
   ...
   * <p>
   * There is usually little need to use this constructor. The period type is
   * used primarily to define how to split an interval into a period. As this
   * constructor already is split, the period type does no real work.
   ...
   * @throws IllegalArgumentException if an unsupported field's value is
   *           non-zero
   */
  ...
  /**
   * Creates a period from the given millisecond duration using the standard set
   * of fields.
   * <p>
   * Only precise fields in the period type will be used. For the standard
   * period type this is the time fields only. Thus the year, month, week and
   ...
   * <p>
   * Only precise fields in the period type will be used. Imprecise fields will
   * not be populated.
   * <p>
   * If the duration is small then this method will perform as you might expect
   * and split the fields evenly.
   * <p>
   * If the duration is large then all the remaining duration will be stored in
   * the largest available precise field. For details as to which fields are
   * precise, review the period type javadoc.
   * @param duration the duration, in milliseconds
   ...
   * precise, review the period type javadoc.
   * @param duration the duration, in milliseconds
   * @param chronology the chronology to use to split the duration, null means
   *          ISO default
   */
  ...
  /**
   * Creates a period from the given interval endpoints using the standard set
   * of fields.
   ...
   * The two partials must contain the same fields, thus you can specify two
   * <code>LocalDate</code> objects, or two <code>LocalTime</code> objects, but
   * not one of each. As these are Partial objects, time zones have no effect on
   * the result.
   * <p>
   * The two partials must also both be contiguous - see
   * {@link DateTimeUtils#isContiguous(ReadablePartial)} for a definition. Both
   * <code>LocalDate</code> and <code>LocalTime</code> are contiguous.
   * <p>
   * An alternative way of constructing a Period from two Partials is
   * {@link #fieldDifference(ReadablePartial, ReadablePartial)}. That method
   * handles all kinds of partials.
   * @param start the start of the period, must not be null
   * @param end the end of the period, must not be null
   * @throws IllegalArgumentException if the partials are null or invalid
   * @since 1.1
   ...
   * {@link org.joda.time.convert.ConverterManager ConverterManager} and include
   * ReadablePeriod, ReadableInterval and String. The String formats are
   * described by {@link ISOPeriodFormat#standard()}.
  ...
  //-----------------------------------------------------------------------
  /**
   * Get this period as an immutable <code>Period</code> object by returning
   * <code>this</code>.
  ...
  //-----------------------------------------------------------------------
  /**
   * Creates a new Period instance with the same field values but different
   * PeriodType.
   * <p>
   ...
   * <p>
   * Each field of the period is added separately. Thus a period of 2 hours 30
   * minutes plus 3 hours 40 minutes will produce a result of 5 hours 70 minutes
   * - see {@link #normalizedStandard()}.
   * <p>
   * If the period being added contains a non-zero amount for a field that is
   * not supported in this period then an exception is thrown.
   * <p>
   * This period instance is immutable and unaffected by this method call.
   ...
   * <p>
   * Each field of the period is subtracted separately. Thus a period of 3 hours
   * 30 minutes minus 2 hours 40 minutes will produce a result of 1 hour and -10
   * minutes - see {@link #normalizedStandard()}.
   * <p>
   * If the period being added contains a non-zero amount for a field that is
   * not supported in this period then an exception is thrown.
   * <p>
  ...
  //-----------------------------------------------------------------------
  /**
   * Converts this period to a period in weeks assuming a 7 day week, 24 hour
   ...
   * all days are 24 hours, all hours are 60 minutes and all minutes are 60
   * seconds. This is not true when daylight savings time is considered, and may
   * also not be true for some unusual chronologies. However, it is included as
   ...
   * @throws UnsupportedOperationException if the period contains years or
   *           months
   * @throws ArithmeticException if the number of weeks is too large to be
   *           represented
   * @since 1.5
   */
  ...
  /**
   * Converts this period to a period in days assuming a 7 day week, 24 hour
   * day, 60 minute hour and 60 second minute.
   * <p>
   * This method allows you to convert between different types of period.
   * However to achieve this it makes the assumption that all weeks are 7 days,
  ...
  /**
   * Converts this period to a period in hours assuming a 7 day week, 24 hour
   * day, 60 minute hour and 60 second minute.
   * <p>
   * This method allows you to convert between different types of period.
   * However to achieve this it makes the assumption that all weeks are 7 days,
  ...
  /**
   * Converts this period to a period in minutes assuming a 7 day week, 24 hour
   * day, 60 minute hour and 60 second minute.
   * <p>
   * This method allows you to convert between different types of period.
   * However to achieve this it makes the assumption that all weeks are 7 days,
  ...
  /**
   * Converts this period to a period in seconds assuming a 7 day week, 24 hour
   * day, 60 minute hour and 60 second minute.
   * <p>
   * This method allows you to convert between different types of period.
   * However to achieve this it makes the assumption that all weeks are 7 days,
  ...
  //-----------------------------------------------------------------------
  /**
   * Converts this period to a duration assuming a 7 day week, 24 hour day, 60
   ...
   * are 24 hours, all hours are 60 minutes and all minutes are 60 seconds. This
   * is not true when daylight savings time is considered, and may also not be
   * true for some unusual chronologies. However, it is included as it is a
  ...
  //-----------------------------------------------------------------------
  /**
   * Normalizes this period using standard rules, assuming a 12 month year, 7
   * day week, 24 hour day, 60 minute hour and 60 second minute.
   * <p>
   * This method allows you to normalize a period. However to achieve this it
   * makes the assumption that all years are 12 months, all weeks are 7 days,
   * all days are 24 hours, all hours are 60 minutes and all minutes are 60
   * seconds. This is not true when daylight savings time is considered, and may
   * also not be true for some chronologies. However, it is included as it is a
   * useful operation for many applications and business rules.
   * <p>
   * If the period contains years or months, then the months will be normalized
   * to be between 0 and 11. The days field and below will be normalized as
   * necessary, however this will not overflow into the months field. Thus a
   * period of 1 year 15 months will normalize to 2 years 3 months. But a period
   * of 1 month 40 days will remain as 1 month 40 days.
   * <p>
  ...
  /**
   * Normalizes this period using standard rules, assuming a 12 month year, 7
   * day week, 24 hour day, 60 minute hour and 60 second minute, providing
   * control over how the result is split into fields.
   * <p>
   * This method allows you to normalize a period. However to achieve this it
   * makes the assumption that all years are 12 months, all weeks are 7 days,
   * all days are 24 hours, all hours are 60 minutes and all minutes are 60
   ...
   * of 1 month 40 days will remain as 1 month 40 days.
   * <p>
   * The PeriodType parameter controls how the result is created. It allows you
   * to omit certain fields from the result if desired. For example, you may not
   * want the result to include weeks, in which case you pass in
   * <code>PeriodType.yearMonthDayTime()</code>.
   * @param type the period type of the new period, null means standard type
   * @return a normalized period equivalent to this period
   * @throws ArithmeticException if any field is too large to be represented
   * @throws UnsupportedOperationException if this period contains non-zero
   *           years or months but the specified period type does not support
   *           them
   * @since 1.5
   */
