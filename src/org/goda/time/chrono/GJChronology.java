 ...
 * when this chronology switches can be controlled using the second parameter of
 * the getInstance method. By default this cutover is set to the date the
 ...
 * GregorianCalendar} in that years in BCE are returned correctly. Thus year 1
 * BCE is returned as -1 instead of 1. The yearOfEra field produces results
 * compatible with GregorianCalendar.
 * <p>
...
public final class GJChronology extends AssembledChronology {
  ...
  private static long convertByYear(long instant, Chronology from, Chronology to) {
    return to.getDateTimeMillis(from.year().get(instant), from.monthOfYear().get(instant), from.dayOfMonth().get(instant),
   ...
   * uses a cutover date of October 15, 1582 (Gregorian) 00:00:00 UTC. For this
   * value, October 4, 1582 (Julian) is followed by October 15, 1582
   * (Gregorian).
   * <p>
   * The first day of the week is designated to be
   * {@link org.joda.time.DateTimeConstants#MONDAY Monday}, and the minimum days
   * in the first week of the year is 4.
   * <p>
  ...
  /**
   * Factory method returns instances of the GJ cutover chronology. This uses a
   * cutover date of October 15, 1582 (Gregorian) 00:00:00 UTC. For this value,
   * October 4, 1582 (Julian) is followed by October 15, 1582 (Gregorian).
   * <p>
   * The first day of the week is designated to be
   * {@link org.joda.time.DateTimeConstants#MONDAY Monday}, and the minimum days
   * in the first week of the year is 4.
  ...
  /**
   * Factory method returns instances of the GJ cutover chronology. Any cutover
   ...
   * @param zone the time zone to use, null is default
   * @param gregorianCutover the cutover to use, null means default
   * @param minDaysInFirstWeek minimum number of days in first week of the year;
   *          default is 4
   */
  public static synchronized GJChronology getInstance(DateTimeZone zone, ReadableInstant gregorianCutover, int minDaysInFirstWeek) {
    ...
    if (zone == DateTimeZone.UTC) {
      ...
      chrono = new GJChronology(ZonedChronology.getInstance(chrono, zone), chrono.iJulianChronology, chrono.iGregorianChronology,
  ...
  public long getDateTimeMillis(int year, int monthOfYear, int dayOfMonth, int hourOfDay, int minuteOfHour, int secondOfMinute,
      int millisOfSecond) throws IllegalArgumentException {
    Chronology base;
    if ((base = getBase()) != null) {
    ...
    // Assume date is Gregorian.
    long instant = iGregorianChronology.getDateTimeMillis(year, monthOfYear, dayOfMonth, hourOfDay, minuteOfHour, secondOfMinute,
        millisOfSecond);
    if (instant < iCutoverMillis) {
      // Maybe it's Julian.
  ...
  private class CutoverField extends BaseDateTimeField {
    ...
    /**
     * @param julianField field from the chronology used before the cutover
     *          instant
     * @param gregorianField field from the chronology used at and after the
     *          cutover
  ...
  //-----------------------------------------------------------------------
  /**
   * Cutover field for variable length fields. These fields internally call set
   * whenever add is called. As a result, the same correction applied to set
   * must be applied to add and addWrapField. Knowing when to use this field
   * requires specific knowledge of how the GJ fields are implemented.
   */
