...
/**
 * Wraps another Chronology to impose limits on the range of instants that the
 * fields within a Chronology may support. The limits are applied to both
 * DateTimeFields and DurationFields.
 * <p>
 * Methods in DateTimeField and DurationField throw an IllegalArgumentException
 * whenever given an input instant that is outside the limits or when an attempt
 * is made to move an instant outside the limits.
 * <p>
...
public final class LimitChronology extends AssembledChronology {
  ...
  /**
   * Wraps another chronology, with datetime limits. When withUTC or withZone is
   * called, the returned LimitChronology instance has the same limits, except
   * they are time zone adjusted.
   * @param base base chronology to wrap
   * @param lowerLimit inclusive lower limit, or null if none
   * @param upperLimit exclusive upper limit, or null if none
   * @throws IllegalArgumentException if chronology is null or limits are
   *           invalid
   */
  ...
  /**
   * If this LimitChronology is already UTC, then this is returned. Otherwise, a
   * new instance is returned, with the limits adjusted to the new time zone.
   */
  ...
  /**
   * If this LimitChronology has the same time zone as the one given, then this
   * is returned. Otherwise, a new instance is returned, with the limits
  ...
  private DateTimeField convertField(DateTimeField field, Map<DateTimeField, LimitDateTimeField> converted,
      Map<DurationField, LimitDurationField> convertedDurationFields) {
    if (field == null || !field.isSupported()) {
      return field;
    }
    if (converted.containsKey(field)) {
    ...
    LimitDateTimeField limitField = new LimitDateTimeField(field, convertField(field.getDurationField(), convertedDurationFields),
        convertField(field.getRangeDurationField(), convertedDurationFields), convertField(field.getLeapDurationField(),
            convertedDurationFields));
    converted.put(field, limitField);
    return limitField;
  }
  ...
  //-----------------------------------------------------------------------
  /**
   * A limit chronology is only equal to a limit chronology with the same base
  ...
  public boolean equals(Object obj) {
    ...
    return getBase().equals(chrono.getBase()) && FieldUtils.equals(getLowerLimit(), chrono.getLowerLimit())
  ...
  public String toString() {
    return "LimitChronology[" + getBase().toString() + ", " + (getLowerLimit() == null ? "NoLimit" : getLowerLimit().toString()) + ", "
