...
public final class ZonedChronology extends AssembledChronology {
  ...
  private DateTimeField convertField(DateTimeField field, Map<DateTimeField, ZonedDateTimeField> convertedDateTimeFields,
      Map<DurationField, ZonedDurationField> convertedDurationFields) {
    if (field == null || !field.isSupported()) {
      return field;
    }
    ...
    ZonedDateTimeField zonedField = new ZonedDateTimeField(field, getZone(),
        convertField(field.getDurationField(), convertedDurationFields), convertField(field.getRangeDurationField(),
            convertedDurationFields), convertField(field.getLeapDurationField(), convertedDurationFields));
  ...
  //-----------------------------------------------------------------------
  /**
   * A zoned chronology is only equal to a zoned chronology with the same base
   * chronology and zone.
   ...
   * the new offset is subtracted off. This behavior, if applied to time fields,
   * can nullify or reverse an add when crossing a transition.
   */
   ...
   * <p>
   * This class converts passed in instants to local wall time, and vice versa
   * on output.
   */
  static final class ZonedDateTimeField extends BaseDateTimeField {
    ...
    ZonedDateTimeField(DateTimeField field, DateTimeZone zone, DurationField durationField, DurationField rangeDurationField,
    ...
    public long set(long instant, int value) {
      ...
      if (get(result) != value) {
        throw new IllegalFieldValueException(iField.getType(), new Integer(value), "Illegal instant due to time zone offset transition: "
            + DateTimeFormat.forPattern("yyyy-MM-dd'T'HH:mm:ss.SSS").print(new Instant(localInstant)) + " (" + iZone.getID() + ")");
      }
      return result;
    }
