...
/**
 * Abstract Chronology that enables chronologies to be assembled from a
 * container of fields.
 * <p>
...
public abstract class AssembledChronology extends BaseChronology {
   ...
   * supported fields. If a base chronology is supplied, the field set initially
   * contains references to each base chronology field.
   * <p>
   * Other methods in this class will delegate to the base chronology, if it can
   * be determined that the base chronology will produce the same results as
   * AbstractChronology.
  ...
  /**
   * Invoked by the constructor and after deserialization to allow subclasses to
   * define all of its supported fields. All unset fields default to unsupported
   * instances.
  ...
  private void setFields() {
    ...
    if (iBase == null) {
      ...
      flags = ((iHourOfDay == iBase.hourOfDay() && iMinuteOfHour == iBase.minuteOfHour() && iSecondOfMinute == iBase.secondOfMinute() && iMillisOfSecond == iBase.millisOfSecond())
          ? 1 : 0)
          |

          ((iMillisOfDay == iBase.millisOfDay()) ? 2 : 0) |

          ((iYear == iBase.year() && iMonthOfYear == iBase.monthOfYear() && iDayOfMonth == iBase.dayOfMonth()) ? 4 : 0);
    }
