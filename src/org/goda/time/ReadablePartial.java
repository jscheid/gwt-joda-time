 ...
 * thus a local time.
 * <p>
 * A <code>ReadablePartial</code> supports a subset of those fields on the
 * chronology. It cannot be compared to a <code>ReadableInstant</code>, as it
 * does not fully specify an instant in time. The time it does specify is a
 * local time, and does not include a time zone.
 * <p>
 * A <code>ReadablePartial</code> can be converted to a
 * <code>ReadableInstant</code> using the <code>toDateTime</code> method. This
 * works by providing a full base instant that can be used to 'fill in the gaps'
 * and specify a time zone.
...
public interface ReadablePartial {
  ...
  //-----------------------------------------------------------------------
  /**
   * Compares this partial with the specified object for equality based on the
   * supported fields, chronology and values.
   * <p>
  ...
  //-----------------------------------------------------------------------
  /**
   * Get the value as a String in a recognisable ISO8601 format, only displaying
   * supported fields.
   * <p>
