...
public class FieldUtils {
  ...
  /**
   * Multiply two values to return an int throwing an exception if overflow
   * occurs.
   * @param val1 the first value
   * @param val2 the second value
   * @return the new total
  ...
  public static void verifyValueBounds(String fieldName, int value, int lowerBound, int upperBound) {
    if ((value < lowerBound) || (value > upperBound)) {
      throw new IllegalFieldValueException(fieldName, new Integer(value), new Integer(lowerBound), new Integer(upperBound));
    }
  }

  /**
   * Utility method used by addWrapField implementations to ensure the new value
   * lies within the field's legal value range.
   * @param currentValue the current value of the data, which may lie outside the
   *          wrapped value range
   * @param wrapValue the value to add to current value before wrapping. This
   *          may be negative.
   * @param minValue the wrap range minimum value.
   * @param maxValue the wrap range maximum value. This must be greater than
   *          minValue (checked by the method).
   * @return the wrapped value
   * @throws IllegalArgumentException if minValue is greater than or equal to
   *           maxValue
   */
  ...
  /**
   * Utility method that ensures the given value lies within the field's legal
   * value range.
