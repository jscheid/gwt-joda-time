 ...
 * <p>
 * MutablePeriod is mutable and not thread-safe, unless concurrent threads are
 * not invoking mutator methods.
...
public class MutablePeriod extends BasePeriod implements ReadWritablePeriod, Cloneable, Serializable {
   ...
   * larger fields, such as the UTC or precise types.
   * </ul>
  ...
  /**
   * Sets all the fields in one go from an interval using the ISO chronology and
   * dividing the fields using the period type.
   ...
   * ISOChronology and dividing the fields using the period type.
   * @param startInstant interval start, in milliseconds
   * @param endInstant interval end, in milliseconds
   * @throws ArithmeticException if the set exceeds the capacity of the period
   */
  ...
  /**
   * Sets all the fields in one go from a duration dividing the fields using the
   * period type.
   * <p>
   * When dividing the duration, only precise fields in the period type will be
   * used. For large durations, all the remaining duration will be stored in the
   * largest available precise field.
   ...
   *          unsupported
   * @param minutes amount of minutes to add to this period, which must be zero
   *          if unsupported
   * @param seconds amount of seconds to add to this period, which must be zero
   *          if unsupported
  ...
  public void add(int years, int months, int weeks, int days, int hours, int minutes, int seconds, int millis) {
    setPeriod(FieldUtils.safeAdd(getYears(), years), FieldUtils.safeAdd(getMonths(), months), FieldUtils.safeAdd(getWeeks(), weeks),
        FieldUtils.safeAdd(getDays(), days), FieldUtils.safeAdd(getHours(), hours), FieldUtils.safeAdd(getMinutes(), minutes),
        FieldUtils.safeAdd(getSeconds(), seconds), FieldUtils.safeAdd(getMillis(), millis));
  }

  /**
   * Adds an interval to this one by dividing the interval into fields and
   * calling {@link #add(ReadablePeriod)}.
  ...
  /**
   * Adds a duration to this one by dividing the duration into fields and
   * calling {@link #add(ReadablePeriod)}.
