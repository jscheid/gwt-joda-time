...
public abstract class Chronology {
   ...
   * This chronology is the default, and is suitable for all normal datetime
   * processing. It is <i>unsuitable</i> for historical datetimes before October
   * 15, 1582 as it applies the modern Gregorian calendar rules before that
   * date.
   ...
   * <p>
   * {@link GJChronology} defines all fields using standard meanings. This
   * chronology is intended to be used as a replacement for
   * <code>GregorianCalendar</code>. The Gregorian calendar system is used after
   * October 15, 1582, while the Julian calendar system is used before.
   * <p>
   ...
   * <p>
   * This method uses the standard Julian to Gregorian cutover date of October
   * 15th 1582. If you require a cutover on a different date, then use the
   * factories on <code>GJChronology</code> itself.
   * <p>
   * When dealing solely with dates in the modern era, from 1600 onwards, we
   * recommend using ISOChronology, which is the default.
   ...
   * <p>
   * {@link GregorianChronology} defines all fields using standard meanings. It
   * uses the Gregorian calendar rules <i>for all time</i> (proleptic) thus it
   * is NOT a replacement for <code>GregorianCalendar</code>. For that purpose,
   * you should use {@link #getGJ()}.
   * <p>
   * The Gregorian calendar system defines a leap year every four years, except
   * that every 100 years is not leap, but every 400 is leap.
   * <p>
   ...
   * <p>
   * {@link JulianChronology} defines all fields using standard meanings. It
   * uses the Julian calendar rules <i>for all time</i> (proleptic). The Julian
   * calendar system defines a leap year every four years.
   ...
   * <p>
   * {@link CopticChronology} defines fields sensibly for the Coptic calendar
   * system. The Coptic calendar system defines every fourth year as leap. The
   * year is broken down into 12 months, each 30 days in length. An extra period
   * at the end of the year is either 5 or 6 days in length and is returned as a
   * 13th month. Year 1 in the Coptic calendar began on August 29, 284 CE
   * (Julian). The chronology cannot be used before the first Coptic year.
   ...
   * Chronologies that do not operate in a time zone or are already UTC must
   * return themself.
  ...
  /**
   * Returns a datetime millisecond instant, formed from the given year, month,
   * day, and millisecond values. The set of given values must refer to a valid
   * datetime, or else an IllegalArgumentException is thrown.
   * <p>
   * The default implementation calls upon separate DateTimeFields to determine
   * the result. Subclasses are encouraged to provide a more efficient
   * implementation.
  ...
  /**
   * Returns a datetime millisecond instant, formed from the given year, month,
   * day, hour, minute, second, and millisecond values. The set of given values
   ...
   * @throws IllegalArgumentException if the values are invalid
   */
  public abstract long getDateTimeMillis(int year, int monthOfYear, int dayOfMonth, int hourOfDay, int minuteOfHour, int secondOfMinute,
      int millisOfSecond);

  /**
   * Returns a datetime millisecond instant, from from the given instant, hour,
   * minute, second, and millisecond values. The set of given values must refer
  ...
  //-----------------------------------------------------------------------
  /**
   * ValDateTimes whether the values are valid for the fields of a partial
   * instant.
   * @param partial the partial instant to valDateTime
   * @param values the values to valDateTime, not null, match fields in partial
   * @throws IllegalArgumentException if the instant is invalid
   */
  public abstract void valDateTime(ReadablePartial partial, int[] values);
   ...
   * <p>
   * DayOfWeek values are defined in {@link DateTimeConstants}. They use the ISO
   * definitions, where 1 is Monday and 7 is Sunday.
   * @return DateTimeField or UnsupportedDateTimeField if unsupported
   */
