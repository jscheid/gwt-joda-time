 ...
 * <p>
 * A time interval represents a period of time between two instants. Intervals
 * are inclusive of the start instant and exclusive of the end. The end instant
 * is always greater than or equal to the start instant.
 * <p>
 * Intervals have a fixed millisecond duration. This is the difference between
 * the start and end instants. The duration is represented separately by
 * {@link ReadableDuration}. As a result, intervals are not comparable. To
 * compare the length of two intervals, you should compare their durations.
 * <p>
 * An interval can also be converted to a {@link ReadablePeriod}. This
 * represents the difference between the start and end points in terms of fields
...
public final class Interval extends BaseInterval implements ReadableInterval, Serializable {
   ...
   * chronology in the default time zone.
  ...
  /**
   * Constructs an interval from a start and end instant with the ISO default
   * chronology in the specified time zone.
   * @param startInstant start of this interval, as milliseconds from
   *          1970-01-01T00:00:00Z.
   * @param endInstant end of this interval, as milliseconds from
   *          1970-01-01T00:00:00Z.
  ...
  /**
   * Constructs an interval from a start and end instant with the specified
   * chronology.
   ...
   * @param end end of this interval, null means now
   * @throws IllegalArgumentException if the end is before the start
   * @throws ArithmeticException if the start instant exceeds the capacity of a
   *           long
   */
   ...
   * <p>
   * When forming the interval, the chronology from the instant is used if
   * present, otherwise the chronology of the period is used.
   * @param start start of this interval, null means now
   ...
   * ReadableInterval and String. The String formats are described by
   * {@link ISODateTimeFormat#dateTimeParser()} and
   * {@link ISOPeriodFormat#standard()}, and may be 'datetime/datetime',
  ...
  //-----------------------------------------------------------------------
  /**
   * Get this interval as an immutable <code>Interval</code> object by returning
   * <code>this</code>.
   ...
   * continuum. This method returns the amount of the overlap, only if the
   * intervals actually do overlap. If the intervals do not overlap, then null
   * is returned.
   * <p>
   * When two intervals are compared the result is one of three states: (a) they
   * abut, (b) there is a gap between them, (c) they overlap. The abuts state
   * takes precedence over the other two, thus a zero duration interval at the
   * start of a larger interval abuts and does not overlap.
   * <p>
   * The chronology of the returned interval is the same as that of this
   * interval (the chronology of the interval parameter is not used). Note that
   * the use of the chronology was only correctly implemented in version 1.3.
  ...
  //-----------------------------------------------------------------------
  /**
   * Gets the gap between this interval and another interval. The other interval
   * can be either before or after this interval.
   * <p>
   * Intervals are inclusive of the start instant and exclusive of the end. An
   * interval has a gap to another interval if there is a non-zero duration
   * between them. This method returns the amount of the gap only if the
   * intervals do actually have a gap between them. If the intervals overlap or
   * abut, then null is returned.
   * <p>
   * When two intervals are compared the result is one of three states: (a) they
   * abut, (b) there is a gap between them, (c) they overlap. The abuts state
   * takes precedence over the other two, thus a zero duration interval at the
   ...
   * <p>
   * Intervals are inclusive of the start instant and exclusive of the end. An
   * interval abuts if it starts immediately after, or ends immediately before
   * this interval without overlap. A zero duration interval abuts with itself.
   * <p>
   * When two intervals are compared the result is one of three states: (a) they
   * abut, (b) there is a gap between them, (c) they overlap. The abuts state
   * takes precedence over the other two, thus a zero duration interval at the
   ...
   * @param period the period to add to the start to get the new end instant,
   *          null means zero
   * @return an interval with the start from this interval and a calculated end
