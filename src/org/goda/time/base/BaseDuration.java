...
import org.goda.time.convert.DurationConverter;
 ...
 * {@link ReadableDuration} interface should be used when different kinds of
 * duration objects are to be referenced.
 * <p>
...
public abstract class BaseDuration extends AbstractDuration implements ReadableDuration, Serializable {
   ...
   * <p>
   * Only precise fields in the period type will be used. At most these are
   * hours, minutes, seconds and millis - the period type may restrict the
   * selection further.
   * <p>
   * For more control over the conversion process, you must pair the duration
   * with an instant, see {@link #toPeriodFrom(ReadableInstant, PeriodType)}.
   ...
   * <p>
   * Only precise fields in the period type will be used. Exactly which fields
   * are precise depends on the chronology. Only the time fields are precise for
   * ISO chronology with a time zone. However, ISO UTC also has precise days and
   * weeks.
   * <p>
   * For more control over the conversion process, you must pair the duration
   * with an instant, see {@link #toPeriodFrom(ReadableInstant)} and
   ...
   * <p>
   * For more control over the conversion process, you must pair the duration
   * with an instant, see {@link #toPeriodFrom(ReadableInstant, PeriodType)} and
  ...
  /**
   * Converts this duration to a Period instance by adding the duration to a
   * start instant to obtain an interval using the standard period type.
   * <p>
   * This conversion will determine the fields of a period accurately. The
   * results are based on the instant millis, the chronology of the instant, the
   * standard period type and the length of this duration.
  ...
  /**
   * Converts this duration to a Period instance by adding the duration to a
   * start instant to obtain an interval.
   * <p>
   * This conversion will determine the fields of a period accurately. The
   * results are based on the instant millis, the chronology of the instant, the
   * period type and the length of this duration.
   * @param startInstant the instant to calculate the period from, null means now
   * @param type the period type determining how to split the duration into
   *          fields, null means All type
   * @return a Period created using the millisecond duration from this instance
   */
