...
public abstract class AbstractPeriod implements ReadablePeriod {
   ...
   * <p>
   * The fields are returned largest to smallest, for example Hours, Minutes,
   * Seconds.
   ...
   * <p>
   * The fields are returned largest to smallest, for example Hours, Minutes,
   * Seconds. Each value corresponds to the same array index as
   * <code>getFields()</code>
   ...
   * <p>
   * If the field type specified is not supported by the period then zero is
   * returned.
   ...
   * value of each field. All ReadablePeriod instances are accepted.
   * <p>
   * Note that a period of 1 day is not equal to a period of 24 hours, nor is 1
   * hour equal to 60 minutes. Only periods with the same amount in each field
   * are equal.
   * <p>
   ...
   * {@link org.joda.time.Duration Duration}s, an operation that emphasises that
   * the result may differ according to the date you choose.
