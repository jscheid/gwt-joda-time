...
/**
 * StringConverter converts from a String to an instant, partial, duration,
 * period or interval..
 * @author Stephen Colebourne
 * @author Brian S O'Neill
 * @since 1.0
 */
class StringConverter extends AbstractConverter implements InstantConverter, PartialConverter, DurationConverter, PeriodConverter,
    IntervalConverter {
   ...
   * This method checks if the parser has a zone, and uses it if present. This
   * is most useful for parsing local times with UTC.
   * @param fieldSource a partial that provides access to the fields. This
   *          partial may be incomplete and only getFieldType(int) should be
   *          used
   * @param object the object to convert
   * @param chrono the chronology to use, which is the non-null result of
   *          getChronology()
   * @param parser the parser to use, may be null
   * @return the array of field values that match the fieldSource, must be
   *         non-null valid
  ...
  //-----------------------------------------------------------------------
  /**
   * Gets the duration of the string using the standard type. This matches the
   * toString() method of ReadableDuration.
   * @param object the String to convert, must not be null
   * @throws ClassCastException if the object is invalid
   */
  public long getDurationMillis(Object object) {
    ...
    String str = original;
    int len = str.length();
    if (len >= 4 && (str.charAt(0) == 'P' || str.charAt(0) == 'p') && (str.charAt(1) == 'T' || str.charAt(1) == 't')
  ...
  //-----------------------------------------------------------------------
  /**
   * Extracts duration values from an object of this converter's type, and sets
   * them into the given ReadWritableDuration.
