...
public class DateTimeFormatterBuilder {
   ...
   * methods {@link DateTimeFormatter#isPrinter()} and
   * {@link DateTimeFormatter#isParser()} will help you determine the state of
   * the formatter.
   * @throws UnsupportedOperationException if neither printing nor parsing is
   *           supported
   */
  ...
  /**
   * Internal method to create a DateTimePrinter instance using all the appended
   * elements.
   * <p>
  ...
  /**
   * Internal method to create a DateTimeParser instance using all the appended
   * elements.
   * <p>
   * Most applications will not use this method. If you want a parser in an
   * application, call {@link #toFormatter()} and just use the parsing API.
   * <p>
  ...
  /**
   * Appends just a printer. With no matching parser, a parser cannot be built
   * from this DateTimeFormatterBuilder.
  ...
  /**
   * Appends just a parser. With no matching printer, a printer cannot be built
   * from this builder.
   ...
   * @param parser the parser to add
   * @return this DateTimeFormatterBuilder
   * @throws IllegalArgumentException if printer or parser is null or of an
   *           invalid type
   */
   ...
   * last of the parser array elements to be null. If the last element is null,
   * this represents the empty parser. The presence of an empty parser indicates
   * that the entire array of parse formats is optional.
   * @param printer the printer to add
   * @param parsers the parsers to add
   * @return this DateTimeFormatterBuilder
   * @throws IllegalArgumentException if any printer or parser is of an invalid
   *           type
  ...
  /**
   * Appends just a parser element which is optional. With no matching printer,
   * a printer cannot be built from this DateTimeFormatterBuilder.
   * @return this DateTimeFormatterBuilder
   * @throws IllegalArgumentException if parser is null or of an invalid type
   */
  ...
  /**
   * Instructs the printer to emit a field value as a fixed-width decimal number
   * (smaller numbers will be left-padded with zeros), and the parser to expect
   * an unsigned decimal number with the same fixed width.
   * @param fieldType type of field to append
   * @param numDigits the exact number of digits to parse or print, except if
   *          printed value requires more digits
   * @return this DateTimeFormatterBuilder
   * @throws IllegalArgumentException if field type is null or if
   *           <code>numDigits <= 0</code>
   * @since 1.5
   */
   ...
   * Instructs the printer to emit a field value as a fixed-width decimal number
   * (smaller numbers will be left-padded with zeros), and the parser to expect
   * an signed decimal number with the same fixed width.
   * @param fieldType type of field to append
   * @param numDigits the exact number of digits to parse or print, except if
   *          printed value requires more digits
   * @return this DateTimeFormatterBuilder
  ...
  /**
   * Instructs the printer to emit a field value as text, and the parser to
   * expect text.
   * @param fieldType type of field to append
   * @return this DateTimeFormatterBuilder
   * @throws IllegalArgumentException if field type is null
  ...
  /**
   * Instructs the printer to emit a field value as short text, and the parser
   * to expect text.
   * @param fieldType type of field to append
   * @return this DateTimeFormatterBuilder
   * @throws IllegalArgumentException if field type is null
   ...
   * sans decimal point. For example, if the field is specified as minuteOfHour
   * and the time is 12:30:45, the value printed is 75. A decimal point is
   * implied, so the fraction is 0.75, or three-quarters of a minute.
   * @param fieldType type of field to append
   ...
   * parses two digits. A pivot year is used during parsing to determine the
   * range of supported years as <code>(pivot - 50) .. (pivot + 49)</code>.
   * 
   * <pre>
  ...
  /**
   * Instructs the printer to emit a numeric year field which always prints two
   * digits. A pivot year is used during parsing to determine the range of
   * supported years as <code>(pivot - 50) .. (pivot + 49)</code>. If parse is
   * instructed to be lenient and the digit count is not two, it is treated as
   * an absolute year. With lenient parsing, specifying a positive or negative
   * sign before the year also makes it absolute.
   * @param pivot pivot year to use when parsing
   * @param lenientParse when true, if digit count is not two, it is treated as
   *          an absolute year
   * @return this DateTimeFormatterBuilder
   * @since 1.1
   */
   ...
   * and parses two digits. A pivot year is used during parsing to determine the
   * range of supported years as <code>(pivot - 50) .. (pivot + 49)</code>.
   * 
   * <pre>
   * pivot   supported range   00 is   20 is   40 is   60 is   80 is
  ...
  /**
   * Instructs the printer to emit a numeric weekyear field which always prints
   * two digits. A pivot year is used during parsing to determine the range of
   * supported years as <code>(pivot - 50) .. (pivot + 49)</code>. If parse is
   * instructed to be lenient and the digit count is not two, it is treated as
   * an absolute weekyear. With lenient parsing, specifying a positive or
   * negative sign before the weekyear also makes it absolute.
   * @param pivot pivot weekyear to use when parsing
   * @param lenientParse when true, if digit count is not two, it is treated as
   *          an absolute weekyear
   * @return this DateTimeFormatterBuilder
   * @since 1.1
   */
  ...
  /**
   * Instructs the printer to emit a locale-specific AM/PM text, and the parser
   * to expect it. The parser is case-insensitive.
   * @return this DateTimeFormatterBuilder
   */
  ...
  /**
   * Instructs the printer to emit a locale-specific dayOfWeek text. The parser
   * will accept a long or short dayOfWeek text, case-insensitive.
   * @return this DateTimeFormatterBuilder
   */
  ...
  /**
   * Instructs the printer to emit a short locale-specific dayOfWeek text. The
  ...
  /**
   * Instructs the printer to emit a short locale-specific monthOfYear text. The
  ...
  /**
   * Instructs the printer to emit a locale-specific era text (BC/AD), and the
  ...
  /**
   * Instructs the printer to emit a locale-specific time zone name. A parser
   * cannot be created from this builder if a time zone name is appended.
   * @return this DateTimeFormatterBuilder
   */
  ...
  /**
   * Instructs the printer to emit a short locale-specific time zone name. A
   * parser cannot be created from this builder if time zone name is appended.
   * @return this DateTimeFormatterBuilder
   */
  ...
  /**
   * Instructs the printer to emit the identifier of the time zone. This field
   * cannot currently be parsed.
   * @return this DateTimeFormatterBuilder
   */
  ...
  /**
   * Instructs the printer to emit text and numbers to display time zone offset
   * from UTC. A parser will use the parsed time zone offset to adjust the
   * datetime.
   * @param zeroOffsetText Text to use if time zone offset is zero. If null,
   *          offset is always shown.
   * @param showSeparators If true, prints ':' separator before minute and
   *          second field and prints '.' separator before fraction field.
   * @param minFields minimum number of fields to print, stopping when no more
   *          precision is required. 1=hours, 2=minutes, 3=seconds, 4=fraction
  ...
  static class CharacterLiteral implements DateTimePrinter, DateTimeParser {
    ...
    /*
     * public void printTo( Writer out, long instant, Chronology chrono, int
     * displayOffset, DateTimeZone displayZone, Locale locale) throws
     * IOException { out.write(iValue); }
     */
  ...
  static class Fraction implements DateTimePrinter, DateTimeParser {
    ...
    public int parseInto(DateTimeParserBucket bucket, String text, int position) {
      ...
      DateTimeField parseField = new PreciseDateTimeField(DateTimeFieldType.millisOfSecond(), MillisDurationField.INSTANCE,
  ...
  static class TimeZoneOffset implements DateTimePrinter, DateTimeParser {
    ...
    public int parseInto(DateTimeParserBucket bucket, String text, int position) {
      ...
      zeroOffset : if (iZeroOffsetText != null) {
        ...
        if (text.regionMatches(true, position, iZeroOffsetText, 0, iZeroOffsetText.length())) {
          bucket.setOffset(0);
  ...
  static class Composite implements DateTimePrinter, DateTimeParser {
    ...
    /**
     * Processes the element pairs, putting results into the given printer and
     * parser lists.
     */
