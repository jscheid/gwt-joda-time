 ...
 * <p>
 * This class is the main API for printing and parsing used by most
 * applications. Instances of this class are created via one of three factory
 * classes:
 * <ul>
 ...
 * </ul>
 * <p>
 * An instance of this class holds a reference internally to one printer and one
 * parser. It is possible that one of these may be null, in which case the
 * formatter cannot print/parse. This can be checked via the
 * {@link #isPrinter()} and {@link #isParser()} methods.
 * <p>
 * The underlying printer/parser can be altered to behave exactly as required by
 * using one of the decorator modifiers:
 * <ul>
 * <li>{@link #withLocale(Locale)} - returns a new formatter that uses the
 * specified locale</li>
 * <li>{@link #withZone(DateTimeZone)} - returns a new formatter that uses the
 * specified time zone</li>
 * <li>{@link #withChronology(Chronology)} - returns a new formatter that uses
 * the specified chronology</li>
 * <li>{@link #withOffsetParsed()} - returns a new formatter that returns the
 * parsed time zone offset</li>
 * </ul>
 * Each of these returns a new formatter (instances of this class are
 * immutable).
 * <p>
...
public class DateTimeFormatter {
  ...
  /**
   * Creates a new formatter, however you will normally use the factory or the
   * builder.
  ...
  private DateTimeFormatter(DateTimePrinter printer, DateTimeParser parser, Locale locale, boolean offsetParsed, Chronology chrono,
      DateTimeZone zone, Integer pivotYear) {
    super();
    iPrinter = printer;
    iParser = parser;
    iLocale = locale;
  ...
  //-----------------------------------------------------------------------
  /**
   * Returns a new formatter with a different locale that will be used for
   * printing and parsing.
   * <p>
   * A DateTimeFormatter is immutable, so a new instance is returned, and the
   * original is unaltered and still usable.
   * @param locale the locale to use; if null, formatter uses default locale at
   *          invocation time
  ...
  //-----------------------------------------------------------------------
  /**
   * Returns a new formatter that will create a datetime with a time zone equal
   * to that of the offset of the parsed string.
   * <p>
   * After calling this method, a string '2004-06-09T10:20:30-08:00' will create
   * a datetime with a zone of -08:00 (a fixed zone, with no daylight savings
   * rules). If the parsed string represents a local time (no zone offset) the
   * parsed datetime will be in the default zone.
   * <p>
   * Calling this method sets the override zone to null. Calling the override
   * zone method sets this flag off.
   * @return the new formatter
   */
  ...
  /**
   * Checks whether the offset from the string is used as the zone of the parsed
   * datetime.
   ...
   * <p>
   * A null chronology means no-override. If both an override chronology and an
   * override zone are set, the override zone will take precedence over the zone
   * in the chronology.
  ...
  //-----------------------------------------------------------------------
  /**
   * Returns a new formatter that will use the specified zone in preference to
   * the zone of the printed object, or default zone on a parse.
   * <p>
   * When printing, this zone will be used in preference to the zone from the
   * datetime that would otherwise be used.
   * <p>
   * When parsing, this zone will be set on the parsed datetime.
   * <p>
   * A null zone means of no-override. If both an override chronology and an
   * override zone are set, the override zone will take precedence over the zone
   * in the chronology.
   ...
   * <p>
   * This setting is useful for changing the pivot year of formats built using a
   * pattern - {@link DateTimeFormat#forPattern(String)}.
   * <p>
   * When parsing, this pivot year is used. Null means no-override. There is no
   * effect when printing.
   * <p>
   * The pivot year enables a two digit year to be converted to a four digit
   * year. The pivot represents the year in the middle of the supported range of
   * years. Thus the full range of years that will be built is
   * <code>(pivot - 50) .. (pivot + 49)</code>.
   * 
   * <pre>
   ...
   * { long millis = DateTimeUtils.getInstantMillis(instant); Chronology chrono
   * = DateTimeUtils.getInstantChronology(instant); printTo(out, millis,
   * chrono); }
   */

  //-----------------------------------------------------------------------
  /**
   * Prints an instant from milliseconds since 1970-01-01T00:00:00Z, using ISO
   * chronology in the default DateTimeZone.
   * @param buf formatted instant is appended to this buffer
   ...
   * <p>
   * Neither the override chronology nor the override zone are used by this
   * method.
   ...
   * { DateTimePrinter printer = requirePrinter(); if (partial == null) { throw
   ...
   * <p>
   * This method will use the override zone and the override chronololgy if they
   * are set. Otherwise it will use the chronology and zone of the instant.
   * @param instant instant to format, null means now
   ...
   * <p>
   * This method will use the override zone and the override chronololgy if they
   * are set. Otherwise it will use the ISO chronology and default zone.
   * @param instant millis since 1970-01-01T00:00:00Z
   * @return the printed result
   */
  ...
  /*
   * private void printTo(Writer buf, long instant, Chronology chrono) throws
   * IOException { DateTimePrinter printer = requirePrinter(); chrono =
   * selectChronology(chrono); // Shift instant into local time (UTC) to avoid
   * excessive offset // calculations when printing multiple fields in a
   * composite printer. DateTimeZone zone = chrono.getZone(); int offset =
   * zone.getOffset(instant); long adjustedInstant = instant + offset; if
   * ((instant ^ adjustedInstant) < 0 && (instant ^ offset) >= 0) { // Time zone
   * offset overflow, so revert to UTC. zone = DateTimeZone.UTC; offset = 0;
   * adjustedInstant = instant; } printer.printTo(buf, adjustedInstant,
   * chrono.withUTC(), offset, zone, iLocale); }
   */
  /**
   ...
   * fields before calling this method, or use {@link #parseDateTime(String)} or
   * {@link #parseMutableDateTime(String)}.
   * <p>
   * If it fails, the return value is negative, but the instant may still be
   * modified. To determine the position where the parse failed, apply the one's
   ...
   * @return new position, negative value means parse failed - apply complement
  ...
  /**
   * Parses a datetime from the given text, returning the number of milliseconds
   * since the epoch, 1970-01-01T00:00:00Z.
   * <p>
   * The parse will use the ISO chronology, and the default time zone. If the
   * text contains a time zone string then that will be taken into account.
   ...
   * {@link #withOffsetParsed()} has been called, then the resulting DateTime
   * will have a fixed offset based on the parsed time zone. Otherwise the
   * resulting DateTime will have the zone of this formatter, but the parsed
   * zone may have caused the time to be adjusted.
   * @param text the text to parse
