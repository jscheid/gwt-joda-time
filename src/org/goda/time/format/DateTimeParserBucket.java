 ...
 * <p>
 * This class allows fields to be saved in any order, but be physically set in a
 * consistent order. This is useful for parsing against formats that allow field
 * values to contradict each other.
 * <p>
 * Field values are applied in an order where the "larger" fields are set first,
 * making their value less likely to stick. A field is larger than another when
 * it's range duration is longer. If both ranges are the same, then the larger
 * field has the longer duration. If it cannot be determined which field is
 * larger, then the fields are set in the order they were saved.
 * <p>
...
public class DateTimeParserBucket {
  ...
  //-----------------------------------------------------------------------
  /**
   * Returns the time zone used by computeMillis, or null if an offset is used
   * instead.
   */
  ...
  /**
   * Set a time zone to be used when computeMillis is called, which overrides
   * any set time zone offset.
  ...
  //-----------------------------------------------------------------------
  /**
   * Returns the time zone offset in milliseconds used by computeMillis, unless
   ...
   * <p>
   * If the value is set to null, this will indicate that default behaviour
   * should be used.
   ...
   * restoreState to undo any changes that were made since the state was saved.
  ...
  /**
   * Restores the state of this bucket from a previously saved state. The state
   * object passed into this method is not consumed, and it can be used later to
   * restore to that state again.
   ...
   * Computes the parsed datetime by setting the saved fields. This method is
   * idempotent, but it is not thread-safe.
   * @param resetFields false by default, but when true, unsaved field values are
   *          cleared
   * @return milliseconds since 1970-01-01T00:00:00Z
   * @throws IllegalArgumentException if any field is out of range
   */
   ...
   * doesn't need to. If the array slice is small enough, an insertion sort is
   * chosen instead, but it doesn't need a copy!
   * <p>
   ...
   * Since the amount of saved fields is small, the insertion sort is a better
   * choice. Additional performance is gained since there is no extra array
   * allocation and copying. Also, the insertion sort here does not perform any
   * casting operations. The version in java.util.Arrays performs casts within
   * the insertion sort loop.
   */
  ...
  static class SavedField implements Comparable {
    ...
    /**
     * The field with the longer range duration is ordered first, where null is
     * considered infinite. If the ranges match, then the field with the longer
     * duration is ordered first.
     */
