...
import org.goda.time.calendar.TimeZoneList;
 ...
 * files, indirectly calling DateTimeZoneBuilder. The following complex example
 * defines the America/Los_Angeles time zone, with all historical transitions:
 * 
 * <pre>
 * DateTimeZone America_Los_Angeles = new DateTimeZoneBuilder().addCutover(-2147483648, 'w', 1, 1, 0, false, 0).setStandardOffset(-28378000).setFixedSavings(
 *     &quot;LMT&quot;, 0).addCutover(1883, 'w', 11, 18, 0, false, 43200000).setStandardOffset(-28800000).addRecurringSavings(&quot;PDT&quot;, 3600000, 1918,
 *     1919, 'w', 3, -1, 7, false, 7200000).addRecurringSavings(&quot;PST&quot;, 0, 1918, 1919, 'w', 10, -1, 7, false, 7200000).addRecurringSavings(
 *     &quot;PWT&quot;, 3600000, 1942, 1942, 'w', 2, 9, 0, false, 7200000).addRecurringSavings(&quot;PPT&quot;, 3600000, 1945, 1945, 'u', 8, 14, 0, false,
 *     82800000).addRecurringSavings(&quot;PST&quot;, 0, 1945, 1945, 'w', 9, 30, 0, false, 7200000).addRecurringSavings(&quot;PDT&quot;, 3600000, 1948, 1948, 'w',
 *     3, 14, 0, false, 7200000).addRecurringSavings(&quot;PST&quot;, 0, 1949, 1949, 'w', 1, 1, 0, false, 7200000).addRecurringSavings(&quot;PDT&quot;, 3600000,
 *     1950, 1966, 'w', 4, -1, 7, false, 7200000).addRecurringSavings(&quot;PST&quot;, 0, 1950, 1961, 'w', 9, -1, 7, false, 7200000).addRecurringSavings(
 *     &quot;PST&quot;, 0, 1962, 1966, 'w', 10, -1, 7, false, 7200000).addRecurringSavings(&quot;PST&quot;, 0, 1967, 2147483647, 'w', 10, -1, 7, false, 7200000).addRecurringSavings(
 *     &quot;PDT&quot;, 3600000, 1967, 1973, 'w', 4, -1, 7, false, 7200000).addRecurringSavings(&quot;PDT&quot;, 3600000, 1974, 1974, 'w', 1, 6, 0, false, 7200000).addRecurringSavings(
 *     &quot;PDT&quot;, 3600000, 1975, 1975, 'w', 2, 23, 0, false, 7200000).addRecurringSavings(&quot;PDT&quot;, 3600000, 1976, 1986, 'w', 4, -1, 7, false,
 *     7200000).addRecurringSavings(&quot;PDT&quot;, 3600000, 1987, 2147483647, 'w', 4, 1, 7, true, 7200000).toDateTimeZone(&quot;America/Los_Angeles&quot;);
 * </pre>
...
public class DateTimeZoneBuilder {
  ...
  //    }
  /**
   * Millisecond encoding formats: upper two bits units field length approximate
   * range --------------------------------------------------------------- 00 30
   * minutes 1 byte +/- 16 hours 01 minutes 4 bytes +/- 1020 years 10 seconds 5
   * bytes +/- 4355 years 11 millis 9 bytes +/- 292,000,000 years Remaining bits
   * in field form signed offset from 1970-01-01T00:00:00Z.
   */
  ...
  /**
   * Adds a cutover for added rules. The standard offset at the cutover defaults
   * to 0. Call setStandardOffset afterwards to change it.
   ...
   * @param advanceDayOfWeek if dayOfMonth does not fall on dayOfWeek, advance
   *          to dayOfWeek when true, retreat when false.
   * @param millisOfDay additional precision for specifying time of day of
   *          cutover
   */
  public DateTimeZoneBuilder addCutover(int year, char mode, int monthOfYear, int dayOfMonth, int dayOfWeek, boolean advanceDayOfWeek,
      int millisOfDay) {
   ...
   * @param mode 'u' - transitions are calculated against UTC, 'w' - transitions
   *          are calculated against wall offset, 's' - transitions are
   ...
   *          transitions
   */
  public DateTimeZoneBuilder addRecurringSavings(String nameKey, int saveMillis, int fromYear, int toYear, char mode, int monthOfYear,
      int dayOfMonth, int dayOfWeek, boolean advanceDayOfWeek, int millisOfDay) {
    if (fromYear <= toYear) {
      OfYear ofYear = new OfYear(mode, monthOfYear, dayOfMonth, dayOfWeek, advanceDayOfWeek, millisOfDay);
  ...
  /**
   * Encodes a built DateTimeZone to the given stream. Call readFrom to decode
   * the data into a DateTimeZone object.
  ...
  private static final class OfYear {
    ...
    /*
     * public String toString() { return "[OfYear]\n" + "Mode: " + iMode + '\n'
    ...
    /**
     * If month-day is 02-29 and year isn't leap, retreats to previous leap
     * year.
     */
  ...
  private static final class RuleSet {
    ...
    /**
     * Returns a transition at firstMillis with the first name key and offsets
     * for this rule set. This method may return null.
    ...
    /**
     * Returns null if RuleSet is exhausted or upper limit reached. Calling this
     * method will throw away rules as they each become exhausted. Copy the
     * RuleSet before using it to compute transitions. Returned transition may
     * be a duplicate from previous transition. Caller must call
  ...
  private static final class DSTZone extends DateTimeZone {
    ...
    public boolean equals(Object obj) {
      ...
      if (obj instanceof DSTZone) {
        ...
        return getID().equals(other.getID()) && iStandardOffset == other.iStandardOffset && iStartRecurrence.equals(other.iStartRecurrence)
  ...
  private static final class PrecalculatedZone extends DateTimeZone {
    ...
    static PrecalculatedZone create(String id, boolean outputID, List<Transition> transitions, DSTZone tailZone) {
      ...
      for (int i = 0; i < nameKeys.length - 1; i++) {
        ...
        if (curOffset != nextOffset && curStdOffset == nextStdOffset && curNameKey.equals(nextNameKey) && p.getYears() == 0
            && p.getMonths() > 4 && p.getMonths() < 8 && curNameKey.equals(zoneNameData[2]) && curNameKey.equals(zoneNameData[4])) {
      ...
      if (tailZone != null) {
        if (tailZone.iStartRecurrence.getNameKey().equals(tailZone.iEndRecurrence.getNameKey())) {
          ...
          if (tailZone.iStartRecurrence.getSaveMillis() > 0) {
            tailZone = new DSTZone(tailZone.getID(), tailZone.iStandardOffset, tailZone.iStartRecurrence.renameAppend("-Summer"),
    ...
    public boolean equals(Object obj) {
      ...
      if (obj instanceof PrecalculatedZone) {
        ...
        return getID().equals(other.getID()) && Arrays.equals(iTransitions, other.iTransitions)
            && Arrays.equals(iNameKeys, other.iNameKeys) && Arrays.equals(iWallOffsets, other.iWallOffsets)
