 ...
 * <p>
 * A time zone is a system of rules to convert time from one geographic location
 * to another. For example, Paris, France is one hour ahead of London, England.
 * Thus when it is 10:00 in London, it is 11:00 in Paris.
 * <p>
 ...
 * UTC-07:00 in the summer. The offset -08:00 indicates that America/Los_Angeles
 * time is obtained from UTC by adding -08:00, that is, by subtracting 8 hours.
 * <p>
 * The offset differs in the summer because of daylight saving time, or DST. The
 * folowing definitions of time are generally used:
 * <ul>
 * <li>UTC - The reference time.
 * <li>Standard Time - The local time without a daylight saving time offset. For
 * example, in Paris, standard time is UTC+01:00.
 * <li>Daylight Saving Time - The local time with a daylight saving time offset.
 * This offset is typically one hour, but not always. It is typically used in
 * most countries away from the equator. In Paris, daylight saving time is
 * UTC+02:00.
 ...
 * However, the factory that accepts a TimeZone will attempt to convert from the
 * old short id to a suitable long id.
 * <p>
...
public abstract class DateTimeZone implements Serializable {
  ...
  static {
    ...
    try {
      ...
      if (cDefault == null) {
        ///
        cDefault = forTimeZone(com.google.gwt.i18n.client.DateTimeFormat.getFormat("zzz").format(new Date()));
      }
   ...
   * ids, as accepted by {@link java.util.TimeZone}, are not accepted. All IDs
   * must be specified in the long format. The exception is UTC, which is an
   * acceptable id.
   * <p>
   * Alternatively a locale independent, fixed offset, datetime zone can be
   * specified. The form <code>[+-]hh:mm</code> can be used.
  ...
  /**
   * Gets a time zone instance for the specified offset to UTC in hours. This
   * method assumes standard length hours.
   * <p>
   ...
   * minutes. This method assumes 60 minutes in an hour, and standard length
   * minutes.
   * <p>
   * This factory is a convenient way of constructing zones with a fixed offset.
   * The minutes value is always positive and in the range 0 to 59. If
   * constructed with the values (-2, 30), the resultiong zone is '-02:30'.
   * @param hoursOffset the offset in hours from UTC
   * @param minutesOffset the offset in minutes from UTC, must be between 0 and
   *          59 inclusive
   * @return the DateTimeZone object for the offset
   * @throws IllegalArgumentException if the offset or minute is too large or
   *           too small
   */
   ...
   * <p>
   * DateTimeZone only accepts a subset of the IDs from TimeZone. The excluded
   * IDs are the short three letter form (except UTC). This method will attempt
   * to convert between time zones created using the short IDs and the full
   * version.
   * <p>
   ...
   * <p>
   * The zone provider is a pluggable instance factory that supplies the actual
   * instances of DateTimeZone.
   * @return the provider
   */
   ...
   * <code>org/joda/time/tz/data</code>. Then uses <code>UTCProvider</code>.
   ...
   * <p>
   * The name provider is a pluggable instance factory that supplies the names
   ...
   * <code>org.joda.time.DateTimeZone.NameProvider</code>. Then uses
  ...
  /**
   * Gets the short name of this datetime zone suitable for display using the
   * default locale.
   * <p>
  ...
  /**
   * Gets the long name of this datetime zone suitable for display using the
   * default locale.
   * <p>
   * If the name is not available for the locale, then this method returns a
   * string in the format <code>[+-]hh:mm</code>.
  ...
  /**
   * Gets the standard millisecond offset to add to UTC to get local time, when
   ...
   * <p>
   * This method can be used to determine whether Summer Time (DST) applies. As
   * a general rule, if the offset at the specified instant is standard, then
   * either Winter time applies, or there is no Summer Time. If the instant is
   * not standard, then Summer Time applies.
   * <p>
   ...
   * <p>
   * This method will return an offset suitable for calculating an instant after
   * any DST gap. For example, consider a zone with a cutover from 01:00 to
   * 01:59:<br />
   ...
   * <p>
   * NOTE: The behaviour of this method changed in v1.5, with the emphasis on
   * returning a consistent result later along the time-line (shown above).
  ...
  /**
   * Converts a standard UTC instant to a local instant with the same local
   * time. This conversion is used before performing a calculation so that the
   * calculation can be done using a simple local zone.
  ...
  /**
   * Converts a local instant to a standard UTC instant with the same local
   * time. This conversion is used after performing a calculation where the
   * calculation was done using a simple local zone.
        ...
        if (nextLocal != nextAdjusted) {
          ...
          if (strict) {
            ...
            throw new IllegalArgumentException("Illegal instant due to time zone offset transition: "
                + DateTimeFormat.forPattern("yyyy-MM-dd'T'HH:mm:ss.SSS").print(new Instant(instantLocal)) + " (" + getID() + ")");
          } else {
   ...
   * @return the UTC millisecond instant with the same local time in the new
   *         zone
   */
   ...
   * If the instant returned is exactly the same as passed in, then no changes
   * occur after the given instant.
   ...
   * If the instant returned is exactly the same as passed in, then no changes
   * occur before the given instant.
   * @param instant milliseconds from 1970-01-01T00:00:00Z
   * @return milliseconds from 1970-01-01T00:00:00Z
   */
   ...
   * referring to the id is written out. When the stub is read in, it replaces
   * itself with a DateTimeZone object.
