...
import org.goda.time.LocalTime;
 ...
 */
public class TestUnsupportedDateTimeField extends GWTTestCase {
  @Override
   ...
   * different duration fields Then the field returned in the first invocation
   * should not be equal to the field returned by the second invocation. In
   * otherwords, the generated instance should be the same for a unique pairing
   * of DateTimeFieldType/DurationField
   */
  public void testDifferentDurationReturnDifferentObjects() {

    /**
     * The fields returned by getInstance should be the same when the duration
     * is the same for both method calls.
     */
  ...
  /**
   * As this is an unsupported date/time field, some normal methods will always
   * return false, as they are not supported. Verify that each method correctly
   * returns null.
   */
  ...
  /**
   * According to the JavaDocs, there are two methods that should always return
   * null. * getRangeDurationField() * getLeapDurationField() Ensure that these
   * are in fact null.
   */
   ...
   * method correctly throws this exception. * add(ReadablePartial instant, int
   * fieldIndex, int[] values, int valueToAdd) * addWrapField(long instant, int
   * value) * addWrapField(ReadablePartial instant, int fieldIndex, int[]
   * values, int valueToAdd) * addWrapPartial(ReadablePartial instant, int
   * fieldIndex, int[] values, int valueToAdd) * get(long instant) *
   * getAsShortText(int fieldValue, Locale locale) * getAsShortText(long
   * instant) * getAsShortText(long instant, Locale locale) *
   * getAsShortText(ReadablePartial partial, int fieldValue, Locale locale) *
   * getAsShortText(ReadablePartial partial, Locale locale) * getAsText(int
   * fieldValue, Locale locale) * getAsText(long instant) * getAsText(long
   * instant, Locale locale) * getAsText(ReadablePartial partial, int
   * fieldValue, Locale locale) * getAsText(ReadablePartial partial, Locale
   * locale) * getLeapAmount(long instant) * getMaximumShortTextLength(Locale
   * locale) * getMaximumTextLength(Locale locale) * getMaximumValue() *
   * getMaximumValue(long instant) * getMaximumValue(ReadablePartial instant) *
   * getMaximumValue(ReadablePartial instant, int[] values) * getMinimumValue()
   * * getMinimumValue(long instant) * getMinimumValue(ReadablePartial instant)
   ...
   * String text, Locale locale) * set(ReadablePartial instant, int fieldIndex,
   * int[] values, int newValue) * set(ReadablePartial instant, int fieldIndex,
   ...
   * Ensure that the toString method returns a string with length greater than 0
   * (and not null)
   */
