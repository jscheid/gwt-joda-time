 ...
 */
public class TestDateTimeZone extends GWTTestCase {
  @Override
  ...
  public void testForID_String() {
    ...
    assertEquals((-7L * DateTimeConstants.MILLIS_PER_HOUR) + (-5L * DateTimeConstants.MILLIS_PER_MINUTE)
        + (-34L * DateTimeConstants.MILLIS_PER_SECOND), zone.getOffset(TEST_TIME_SUMMER));
  ...
  public void testForTimeZone_TimeZone() {
    ...
    DateTimeZone zone = DateTimeZone.forTimeZone("Europe/London");
    assertEquals("Europe/London", zone.getID());
    assertSame(DateTimeZone.UTC, DateTimeZone.forTimeZone("UTC"));

    zone = DateTimeZone.forTimeZone("+00:00");
    assertSame(DateTimeZone.UTC, zone);

    zone = DateTimeZone.forTimeZone("GMT+00:00");
    assertSame(DateTimeZone.UTC, zone);

    zone = DateTimeZone.forTimeZone("GMT+00:00");
    assertSame(DateTimeZone.UTC, zone);

    zone = DateTimeZone.forTimeZone("GMT+00");
    assertSame(DateTimeZone.UTC, zone);

    zone = DateTimeZone.forTimeZone("GMT+01:23");
    assertEquals("+01:23", zone.getID());
    assertEquals(DateTimeConstants.MILLIS_PER_HOUR + (23L * DateTimeConstants.MILLIS_PER_MINUTE), zone.getOffset(TEST_TIME_SUMMER));

    zone = DateTimeZone.forTimeZone("GMT-02:00");
    assertEquals("-02:00", zone.getID());
    assertEquals((-2L * DateTimeConstants.MILLIS_PER_HOUR), zone.getOffset(TEST_TIME_SUMMER));

    zone = DateTimeZone.forTimeZone("EST");
    assertEquals("America/New_York", zone.getID());
  }

  public void testTimeZoneConversion() {
    //FIXME: fix this test  
    //        String jdkTimeZone = "GMT-10";
