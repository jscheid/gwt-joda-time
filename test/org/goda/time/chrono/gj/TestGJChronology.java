 ...
 * purposes only. Correctness is favored over performance. The key functions for
 * date calculations are based on ones provided in "Calendrical Calculations",
 * ISBN 0-521-77752-6.
 * <p>
...
abstract class TestGJChronology extends BaseChronology {
  ...
  long nthWeekday(int n, int weekday, int year, int monthOfYear, int dayOfMonth) {
    if (n > 0) {
      return 7 * n + weekdayBefore(fixedFromGJ(year, monthOfYear, dayOfMonth), weekday);
    } else {
      return 7 * n + weekdayAfter(fixedFromGJ(year, monthOfYear, dayOfMonth), weekday);
    }
  }
